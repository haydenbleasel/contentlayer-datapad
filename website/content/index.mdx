...
```ts title="spacecraft.ts" {1,11-16,27}
function Spacecraft(name, launchYear, missionType) {
  this.name = name;
  this.launchYear = launchYear;
  this.missionType = missionType;

  this.getMissionDuration = function () {
    const currentYear = new Date().getFullYear();
    return currentYear - this.launchYear;
  };
}

// create a new spacecraft
let voyager1 = new Spacecraft('Voyager 1', 1977, 'Interstellar');

console.log(voyager1.name); // outputs: Voyager 1
console.log(voyager1.getMissionDuration()); // outputs: the duration of the mission
```
